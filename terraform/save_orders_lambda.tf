...
data "archive_file" "lambda_zip_saveOrders" {
  type        = "zip"
  output_path = "/tmp/lambda_zip_saveOrders.zip"
  source_dir  = "functions/saveOrders"
}

resource "aws_lambda_function" "saveOrders" {
  function_name = "SaveOrdersTerraform"

  filename         = data.archive_file.lambda_zip_saveOrders.output_path
  source_code_hash = data.archive_file.lambda_zip_saveOrders.output_base64sha256
