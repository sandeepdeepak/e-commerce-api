...
resource "aws_api_gateway_rest_api" "employees_rest_api" {
  depends_on = [aws_cloudformation_stack.e-commerce-api-sam-stack]
  name       = var.app_name
  ...
  body = templatefile("./files/api-def-v1.yaml", {
    ...
    get_order_data_uri         = "${var.lambda_invoke_uri_prefix}/${data.aws_cloudformation_export.api_lambda_arn_cfn_exports["get-order-data"].value}/invocations"
    put_order_data_uri         = "${var.lambda_invoke_uri_prefix}/${data.aws_cloudformation_export.api_lambda_arn_cfn_exports["put-order-data"].value}/invocations"
  })
}
